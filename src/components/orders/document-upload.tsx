                    ...
                    {canUpload && (
                        <div className="mt-4">
                            <Label htmlFor="invoice-upload" className="cursor-pointer">
                                <div className={`border-2 border-dashed rounded-md p-4 flex flex-col items-center justify-center transition-colors
                                    ...
                                    {uploading === "invoice" ? (
                                        ...
                                        <>
                                            <Upload className={`h-6 w-6 mb-2 ${requiredDocumentType === 'invoice' ? 'text-red-500' : 'text-primary'}`} />
                                            <span className={`text-sm font-medium ${requiredDocumentType === 'invoice' ? 'text-red-700' : ''}`}>
                                                {requiredDocumentType === 'invoice' ? 'Upload Invoice (Required)' : 'Upload Invoice'}
                                            </span>
                                            <span className={`text-xs mt-1 ${requiredDocumentType === 'invoice' ? 'text-red-600' : 'text-muted-foreground'}`}>
                                                PDF or Image
                                            </span>
                                        </>
                                    )}
            ...
            <Card>
                <CardHeader>
                    <CardTitle className="text-lg flex items-center gap-2">
                        <FileText className="h-5 w-5" />
                        Receiving Documents
                        {isPickup && <Badge variant="secondary" className="ml-2 bg-blue-50 text-blue-700 hover:bg-blue-50">Picked up in store</Badge>}
                    </CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                    {isPickup ? (
                        <div className="text-sm text-muted-foreground p-8 bg-muted/20 border-dashed border rounded-md text-center flex flex-col items-center justify-center gap-2">
                            <ListChecks className="w-8 h-8 text-gray-300" />
                            <p>Receiving documents are not required for Pickup orders.</p>
                        </div>
                    ) : (
                        <>
